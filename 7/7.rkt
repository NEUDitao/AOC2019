...
(define (five-thrusters amps idx input-queue)
  ...
  (define (do-one-op-and-move-on)
    ...
    (define new-comp (apply intcode-computer-with-index/base apply-input))
  ...
  (define (do-beginning-op)
    (define apply-input (append (list-set (output-pair-fields curr-amp)
                                  1 (list (second input-queue) (first input-queue))) (list 0)))
    (define new-comp (apply intcode-computer-with-index/base apply-input))
